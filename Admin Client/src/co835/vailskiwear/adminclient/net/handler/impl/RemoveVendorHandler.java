 ...
 */
public class RemoveVendorHandler implements PacketHandler{
    ...
    public void handle(final Packet pkt){
        ...
        for(final SalesOrder order : vendor.getOrders())
            SalesOrderManager.remove(order);
        final Vendor selected = ClientWindow.get().getVendorsList().getSelected();
        ClientWindow.get().getVendorsList().remove(vendor);
        if(selected != null && selected.getId() == ClientWindow.get().getVendorInfoPanel().getCurrentVendorId())
